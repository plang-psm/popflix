...
async function fetchMovies(input) {
    const res = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=en-US&query=${input}&page=1&include_adult=false`);
    const data = await res.json();
    if(data.results) {
        // Passing data into an array of custom objects.
        setMovieData( data.results.map((movie) => {
            return {
                    key: movie.id,
                    id: movie.id,
  ...
  return (
    <div className='container'>
        ...
        <div className="heading-container">
            ...
            <button className='watchlist-view' onClick={handleWatchlist}>{watchlist === false ? <h3>My Watchlist</h3> : <h3>Home</h3>}</button>
        </div>
